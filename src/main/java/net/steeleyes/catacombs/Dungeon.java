...
public class Dungeon implements Listener {
    ...
    public Dungeon(Catacombs plugin, ResultSet rs) {
        try {
            ...
            if (world == null) {
                ...
                while (rs2.next()) add(new CatLevel(plugin, rs2, world, false));
                ResultSet rs3 = plugin.getSql().query("SELECT fid,type,val FROM flags WHERE did=" + did + ";");
                while (rs3.next()) {
                    CatFlag flag = new CatFlag(plugin, rs3);
                    if (flag.matches(CatFlag.Type.BOSS_KILLED)) bossKilled = flag;
                    else if (flag.matches(CatFlag.Type.IS_ENABLED)) isEnabled = flag;
                    else if (flag.matches(CatFlag.Type.RESET_TIME)) resetTime = flag;
                    else if (flag.matches(CatFlag.Type.RESET_MAX)) resetMax = flag;
                    else if (flag.matches(CatFlag.Type.RESET_MIN)) resetMin = flag;
                    ...
                    } else System.err.println("[Catacombs] ERROR: unrecognised dungeon flag=" + flag);
                }
                ResultSet rs4 = plugin.getSql().query("SELECT xid,type,x,y,z FROM locations WHERE did=" + did + ";");
                while (rs4.next()) {
                    endChest = new CatLocation(rs4);
                }
                // New defaults
                if (floorFlg == null) {
    ...
    public void render(BlockChangeHandler handler) throws Exception {
        ...
        for (CatLevel l : levels) {
            ...
            text[0] = name;
            if (lvl == 0) text[1] = "Levels:" + (levels.size() - 1);
            else text[1] = "Level:" + lvl;
    ...
    public void unrender(Catacombs plugin, BlockChangeHandler handler) {
        allPlayersToTop();
        for (CatLevel l : levels) l.delete(plugin, handler);
    }
    ...
    public void saveDB() {
        if (did <= 0) {
            ...
            if (did <= 0) did = plugin.getSql().getLastId();

            for (CatLevel l : levels) l.saveDB(plugin.getSql(), did);
            setupFlagsLocations();
    ...
    public final void setupFlagsLocations() {
        if (endChest == null)
            endChest = new CatLocation(getEndChest().getX(), getEndChest().getY(), getEndChest().getZ());
        if (bossKilled == null) bossKilled = new CatFlag(CatFlag.Type.BOSS_KILLED, false);
        if (isEnabled == null) isEnabled = new CatFlag(CatFlag.Type.IS_ENABLED, true);
        if (resetTime == null) resetTime = new CatFlag(CatFlag.Type.RESET_TIME, (long) 0);
        if (resetMax == null) resetMax = new CatFlag(CatFlag.Type.RESET_MAX, (long) 0);
        if (roofFlg == null) roofFlg = new CatFlag(CatFlag.Type.ROOF, roofMat.toString());
        if (floorFlg == null) floorFlg = new CatFlag(CatFlag.Type.FLOOR, floorMat.toString());
        if (resetMin == null) resetMin = new CatFlag(CatFlag.Type.RESET_MIN, (long) 0);
    }

    private void updateBBox() {
        for (CatLevel l : levels) {
            Cuboid c = l.getCube();
            if (bbox == null) bbox = new Cuboid(c.xl, c.yl, c.zl, c.xh, c.yh, c.zh);
            else bbox.union(c);
        }
    }
    ...
    public List<String> getinfo() {
        ...
        for (CatLevel l : levels) {
            ...
            for (String s : l.getinfo()) info.add("    " + s);
        }
        return info;
    }
    ...
    public List<String> dump() {
        ...
        List<String> info = new ArrayList<>();
        for (CatLevel l : levels) for (String s : l.dump(hut.getTop())) info.add(s);
        return info;
    }

    public List<String> map() {
        List<String> info = new ArrayList<>();
        for (CatLevel l : levels) for (String s : l.map()) info.add(s);
        return info;
    }
    ...
    public Boolean isOk() {
        for (CatLevel l : levels) if (!l.isOk()) return false;
        return true;
    }

    public ArrayList<String> summary() {
        ArrayList<String> res = new ArrayList<>();
        for (CatLevel l : levels) res.add(l.summary());
        return res;
    }
    ...
    public Boolean overlaps(Dungeon that) {
        ...
        for (CatLevel a : levels) for (CatLevel b : that.levels) if (a.getCube().overlaps(b.getCube())) return true;
        return false;
    }

    public Boolean isProtected(Block blk) {
        if (bbox == null || blk == null || world == null) return false;
        if (bbox.isIn(blk.getX(), blk.getY(), blk.getZ()) &&
                world.equals(blk.getWorld()) &&
                isEnabled != null &&
                isEnabled.getBoolean()) for (CatLevel l : levels) if (l.getCube().isInRaw(blk)) return true;
        return false;
    }
    ...
    public Boolean isInRaw(Block blk) {
        if (bbox == null || blk == null || world == null) return false;
        if (bbox.isIn(blk.getX(), blk.getY(), blk.getZ()) &&
                world.equals(blk.getWorld())) for (CatLevel l : levels) if (l.getCube().isInRaw(blk)) return true;
        return false;
    }
    ...
    public void prospect(Player p, int x, int y, int z, Direction dir, int maxLevels) {
        ...
        if (level.getBuild_ok() && maxLevels > 0) {
            ...
            while (from.getCan_go_lower() && from.getBuild_ok() && levels.size() < maxLevels + 1) {
                ...
                if (tmp_dir != null) tmp_dir = tmp_dir.turn180();
                else tmp_dir = Direction.ANY;
        ...
        if (maxLevels == 0) level.stealDirection(level);
    ...
    public void clearMonsters(Catacombs plugin) {
        for (CatLevel l : levels) l.clearMonsters(plugin);
    }

    public int changeDoorsToIron() {
        int cnt = 0;
        for (CatLevel l : levels) cnt += l.changeDoorsToIron();
        return cnt;
    }

    public int fixSecretDoors() {
        int cnt = 0;
        for (CatLevel l : levels) cnt += l.fixSecretDoors();
        return cnt;
    }

    public int fixDoors() {
        int cnt = 0;
        for (CatLevel l : levels) cnt += l.fixDoors();
        return cnt;
    }
    ...
    public void reset() {
        allPlayersToTopProt();
        setBossKilled(false);
        for (CatLevel l : levels) l.reset(plugin);
    }

    public void suspend() {
        isEnabled.setBoolean(false);
        isEnabled.saveDB(plugin.getSql(), did);
        for (CatLevel l : levels) l.suspend(plugin, roofMat);
    }

    public void buildWindows(Material mat) {
        for (CatLevel l : levels) l.buildWindows(mat);
    }
    ...
    public void enable() {
        ...
        isEnabled.saveDB(plugin.getSql(), did);
        for (CatLevel l : levels) l.enable(roofMat);
    }
    ...
    public void setResetMinMax(String t) {
        ...
        } else lo = hi = CatUtils.parseTime(t);
        resetMax.setLong(hi);
        resetMin.setLong(lo);
        resetMax.saveDB(plugin.getSql(), did);
        resetMin.saveDB(plugin.getSql(), did);
    ...
    public Location getTopLocation() {
        ...
        if (l == null || l.getTop().getBlockY() == 0) return null;
        int depth = l.getRoofDepth() + l.getRoomDepth();
        return getSafePlace(world.getBlockAt(l.getTop().getBlockX(), l.getTop().getBlockY() - depth + 1, l.getTop().getBlockZ()));
    }
    ...
    public void teleport(Player p, Location loc) {
        ...
        if (!w.isChunkLoaded(chunk)) w.loadChunk(chunk);
        else w.refreshChunk(chunk.getX(), chunk.getZ());
        p.teleport(loc);
    }
    ...
    public Boolean isNatural() {
        for (CatLevel l : levels) if (l.getCube().isLevel() && !l.getCube().isNatural(plugin.getCnf())) return false;
        return true;
    }
    ...
    public Boolean isInhabited() {
        for (Player player : world.getPlayers()) {
            ...
            for (CatLevel l : levels) {
                CatCuboid c = l.getCube();
                if (c.isLevel() && c.isIn(x, y, z)) return true;
            }
        }
        return false;
    }
    ...
    public List<Player> allPlayersInRaw() {
        ...
        for (Player player : world.getPlayers()) if (isInRaw(player.getLocation().getBlock())) list.add(player);
        return list;
    }

    public List<Player> allPlayersProtected() {
        List<Player> list = new ArrayList<>();
        for (Player player : world.getPlayers()) if (isProtected(player.getLocation().getBlock())) list.add(player);
        return list;
    }

    private void movePlayers(Location tloc, List<Player> list) {
        for (Player player : list) {
            if (tloc != null) player.teleport(tloc);
            else {
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        ...
        if (isProtected(blk) && !plugin.getCnf().isPlaceable(blk)) event.setCancelled(true);
    }
    ...
    public void onBlockSpread(BlockSpreadEvent event) {
        Block blk = event.getBlock();
        if (isInRaw(blk) && blk.getType() == Material.DIRT && event.getNewState().getType() == Material.GRASS)
            event.setCancelled(true);
    }
    ...
    public void onBlockDamage(BlockDamageEvent event) {
        if (isInRaw(event.getBlock())) CatUtils.toggleSecretDoor(event.getBlock());
    }
    ...
    public void onPlayerBucketEmpty(PlayerBucketEmptyEvent event) {
        if (isProtected(event.getBlockClicked()) && !event.isCancelled()) event.setCancelled(true);
    }
    ...
    public void onPlayerCommandPreprocess(PlayerCommandPreprocessEvent event) {
        ...
        if (isProtected(blk) && !event.isCancelled() &&
                !plugin.hasPermission(player, "catacombs.admin")) for (String cmd : plugin.getCnf().BannedCommands())
        ...
        if (isInRaw(blk) && !event.isCancelled()) {
            ...
            if (mat == Material.STONE_BUTTON) {
                ...
                if (below.getType() == Material.CHEST)
                    if (plugin.getCnf().ResetButton()) plugin.Commands(null, new String[]{"reset", name});
                    else if (plugin.getCnf().RecallButton()) teleportToTop(event.getPlayer());
            } else if (plugin.getCnf().ClickIronDoor() && mat == Material.IRON_DOOR_BLOCK) {
                Block below = blk.getRelative(BlockFace.DOWN);
                if (below.getType() == Material.IRON_DOOR_BLOCK) below.setData((byte) (below.getData() ^ 4));
                if (blk.getRelative(BlockFace.UP).getType() == Material.IRON_DOOR_BLOCK)
    ...
    public void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (plugin.getCnf().NoArmourInDungeon() && !event.getFrom().getBlock().equals(after) && isProtected(after)) {   // Player has moved to a new block
            Player player = event.getPlayer();
            PlayerInventory inv = player.getInventory();
            if ((inv.getBoots() != null ||
                    inv.getChestplate() != null ||
                    inv.getHelmet() != null ||
                    inv.getLeggings() != null) && getBlockDepth(after) > 0) {
    ...
    public void onEntityDamage(EntityDamageEvent evt) {
        ...
        if (isInRaw(blk) && plugin.getCnf().NoPvPInDungeon() &&
                ent instanceof Player &&
                CatUtils.getDamager(evt) instanceof Player) evt.setCancelled(true);
    }
    ...
    public void onCreatureSpawn(CreatureSpawnEvent event) {
        ...
        if (isInRaw(blk) && !event.isCancelled()) {
            ...
            if (isEnabled != null && !isEnabled.getBoolean()) event.setCancelled(true);
            LivingEntity ent = event.getEntity();
            EntityType t = event.getEntityType();
            if (t == EntityType.WOLF) ((Wolf) ent).setAngry(true);
            else if (t == EntityType.PIG_ZOMBIE) ((PigZombie) ent).setAngry(true);
        }
    }
            ...
            if (event instanceof PlayerDeathEvent) {  // Player death
                ...
                if (!plugin.getCnf().GoldOff()) {
                    ...
                    if (damager instanceof Player) {
                        ...
                        if (bal != null && gold > 0) {
                            damager.sendMessage(gold + " coins (" + bal + ")");
                        }
                    }
                }
            }
