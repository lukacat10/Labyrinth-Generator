...
public class Catacombs extends JavaPlugin {
    ...
    public void onEnable() {
        if (!enabled) {
            ...
            this.getServer().getScheduler().scheduleSyncRepeatingTask(this, new Runnable() {
                @Override
                public void run() {
                    for (Dungeon dung : getDungeons().all()) dung.maintain();
                }
            }, 40, 20);
    ...
    private boolean setupEconomy() {
        ...
        if (economyProvider != null) {
            ...
            if (economy != null) System.out.println("[Catacombs] Found economy system '" + economy.getName() + "'");
        }
        return (economy != null);
    }
    ...
    public Boolean Commands(Player p, String[] args) {
        try {
            if (args.length < 1) {
                ...
                if (dungeons.exists(args[1])) dungeons.remove(args[1]);
...
//        p.getWorld().dropItem(p.getLocation(), stk);
    ...
    public void buildDungeon(Player p, String dname) {
        if (dungeons.exists(dname)) try {
            Dungeon dung = dungeons.get(dname);
            Dungeon overlap = dungeons.getOverlap(dung);
            if (dung.isBuilt()) inform(p, "Dungeon " + dname + " has already been built");
            else if (!dung.isNatural()) inform(p, "Loaction of '" + dname + "' is no longer solid-natural (replan)");
        ...
        else inform(p, "Dungeon " + dname + " doesn't exist");
    }
