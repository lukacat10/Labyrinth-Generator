...
public class CatCuboid extends Cuboid {
        ...
        for (int y = yl; y <= yh; y++)
            for (int x = xl; x <= xh; x++)
                for (int z = zl; z <= zh; z++) {
                    ...
                    info.add((x - top.getX()) + "," + (y - top.getY()) + "," + (z - top.getZ()) + "," + blk.getTypeId() + "," + blk.getData());
                }
        return info;
    }
    ...
    public void forceLightLevel(int level) {
        try {
            new scCatCuboid(this).forceLightLevel(level, this.world, this.xl, this.xh, this.yl, this.yh, this.zl, this.zh);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
            ...
            for (int z = zl; z <= zh; z++)
                for (int y = yl; y <= yh; y++) {
                    ...
                    if (blk.getType() == Material.FENCE)
                        if (above.getType() == Material.AIR) above.setType(Material.CAKE_BLOCK);
                        else if (above.getType() == Material.CAKE_BLOCK && above.getData() != (byte) 0)
    ...
    public void addGlow(CatMat mat, int roofDepth) {
        for (int x = xl; x <= xh; x++)
            for (int z = zl; z <= zh; z++) {
                ...
                if (mat.equals(blk) && other.getType() == Material.AIR) blk.setType(Material.GLOWSTONE);
            }
    }
    ...
    public void removeGlow(CatMat mat, int roofDepth) {
        for (int x = xl; x <= xh; x++)
            for (int z = zl; z <= zh; z++) {
                ...
                Block blk = world.getBlockAt(x, y, z);
                Block other = blk.getRelative(BlockFace.DOWN, 1);
                if (blk.getType() == Material.GLOWSTONE && other.getType() == Material.AIR) mat.setBlock(blk);
            }
    }

    public void buildWindows(CatMat mat, int height) {
        if (type == Type.HUT) return;
            ...
            for (int z = zl; z <= zh; z++)
                for (int y = yl; y <= yh; y++) {
                    ...
                    if (blk.getType() == Material.WOODEN_DOOR || blk.getType() == Material.IRON_DOOR_BLOCK) {
                        ...
                        if (data >= 4 && data < 8) blk.setData((byte) (data ^ 4));
                    } else if (blk.getType() == Material.TRAP_DOOR) blk.setData((byte) (blk.getData() & ~4));
                     else if (blk.getType() == Material.LEVER) blk.setData((byte) (blk.getData() & ~8));
                ...
                for (int y = yl; y <= yh; y++) {
                    ...
                    if (blk.getType() == Material.CHEST) {
                        if (type == Type.LEVEL) {  // Leave chests in the hut
                            ...
                            if (isBigChest(blk)) CatLoot.bigChest(config, chest.getInventory());
                            else if (isMidChest(blk)) CatLoot.midChest(config, chest.getInventory());
                            else CatLoot.smallChest(config, chest.getInventory());
                        }
    ...
    public void clearMonsters() {
        for (Entity ent : world.getEntities())
            if (ent instanceof Creature) {
                ...
                if (isIn(blk.getX(), blk.getY(), blk.getZ())) ent.remove();
            }
    }
    ...
    private byte getDoorLowerCode(Block blk) {
        ...
        if (CatUtils.isSolid(e) && CatUtils.isSolid(w)) return 0;

        Block n = blk.getRelative(BlockFace.NORTH);
        Block s = blk.getRelative(BlockFace.SOUTH);
        if (CatUtils.isSolid(n) && CatUtils.isSolid(s)) return 1;
    ...
    private byte getDoorUpperCode(Block blk) {
        Block e = blk.getRelative(BlockFace.EAST);
        Block w = blk.getRelative(BlockFace.WEST);
        if (CatUtils.isSolid(e) && CatUtils.isSolid(w)) return 8;

        Block n = blk.getRelative(BlockFace.NORTH);
        Block s = blk.getRelative(BlockFace.SOUTH);
        if (CatUtils.isSolid(n) && CatUtils.isSolid(s)) return 8;
    ...
    public Boolean isNatural(CatConfig cnf) {
        for (int x = xl; x <= xh; x++)
            for (int y = yl; y <= yh; y++)
                for (int z = zl; z <= zh; z++) if (!cnf.isNatural(world.getBlockAt(x, y, z))) return false;
        return true;
    }
    ...
    public Boolean isCubeAir() {
        for (int x = xl; x <= xh; x++)
            for (int y = yl; y <= yh; y++)
                for (int z = zl; z <= zh; z++) if (world.getBlockAt(x, y, z).getType() != Material.AIR) return false;
        return true;
    }
        ...
        for (int y = yl; y <= yh; y++)
            for (int x = xl; x <= xh; x++)
                for (int z = zl; z <= zh; z++) {
                    ...
                    if ((was == ' ' || was == ',') && (id == 4 || id == 48 || id == 98 || id == 24))
                        grid[x - xl][z - zl] = '#';
