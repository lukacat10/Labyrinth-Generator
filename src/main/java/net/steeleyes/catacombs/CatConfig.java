...
public class CatConfig extends Config implements ICatConfig {
    ...
    public Boolean RecallButton() {
        return getSBoolean(ECatConfig.RecallButton.getStr());
    }
    ...
    private Integer EndermanPct() {
        return getSInt(ECatConfig.EndermanPct.getStr());
    }

    private Integer SlimePct() {
        return getSInt(ECatConfig.SlimePct.getStr());
    }
    ...
    private Integer MobDropReductionPct() {
        return getSInt(ECatConfig.MobDropReductionPct.getStr());
    }
    ...
    private Integer MedSmallPct() {
        return getSInt(ECatConfig.MedSmallPct.getStr());
    }
    ...
    private Integer BigSmallPct() {
        return getSInt(ECatConfig.BigSmallPct.getStr());
    }
    ...
    public Boolean NoTeleportIn() {
        return getSBoolean(ECatConfig.NoTeleportIn.getStr());
    }

    public Boolean NoTeleportOut() {
        return getSBoolean(ECatConfig.NoTeleportOut.getStr());
    }
    ...
    public Boolean NoArmourInDungeon() {
        return getSBoolean(ECatConfig.NoArmourInDungeon.getStr());
    }
    ...
    public Boolean RespawnInHut() {
        return getSBoolean(ECatConfig.RespawnInHut.getStr());
    }
    ...
    public Boolean ClickIronDoor() {
        return getSBoolean(ECatConfig.ClickIronDoor.getStr());
    }
    ...
    private List<String> PlaceBlocks() {
        return getSStringList(ECatConfig.PlaceBlocks.getStr());
    }

    public Boolean SmallEquipChance() {
        return Chance(SmallEquipPct());
    }

    public Boolean MobDropReductionChance() {
        return Chance(MobDropReductionPct());
    }
    ...
    public Boolean MedEquipChance() {
        return Chance(MedEquipPct());
    }

    public Boolean MedSmallChance() {
        return Chance(MedSmallPct());
    }

    public Boolean BigEquipChance() {
        return Chance(BigEquipPct());
    }

    public Boolean BigSmallChance() {
        return Chance(BigSmallPct());
    }

    public Boolean MinorChance() {
        return Chance(MossyPct());
    }
    ...
    public void setRadiusMax(int r) {
        setSInt(ECatConfig.RadiusMax.getStr(), r);
    }

    public int getMaxChangesPerSecond() {
        return getSInt(ECatConfig.maxBlockChangesPerSecond.getStr());
    }
...
//  public int    MySQLPort()       { return getSInt("MySQL.Server.Port"); }  
    ...
    private void setDefaults() {
        for (ECatConfig att : ECatConfig.values()) {
            ...
            if (path.startsWith(".")) path = def_style + path;
            if (fcnf.get(path) == null) fcnf.set(path, att.getDef());
        }
    }
    ...
    private List<CatMat> cacheBlockMaterialList(List<String> in) {
        List<CatMat> out = new ArrayList<>();
        for (String name : in) try {
    ...
    public Boolean isNatural(Block blk) {
        ...
        for (CatMat i : NaturalList) if (mat.equals(i)) return true;
        return false;
    }

    public Boolean isBreakable(Block blk) {
        CatMat mat = new CatMat(blk);
        for (CatMat i : BreakList) if (mat.getMat() == i.getMat()) return true;
        return false;
    }

    public Boolean isPlaceable(Block blk) {
        CatMat mat = new CatMat(blk);
        for (CatMat i : PlaceList) if (mat.getMat() == i.getMat()) return true;
        return false;
    }
    ...
    public Double Gold() {
        ...
        double max = GoldMax();
        double min = GoldMin();
        if (max - min < 0.001) a = min;
        else a = (Math.random() * (max - min)) + min;
        return Round(a, 2);
    }
    ...
    public Material ShroomType() {
        if (Chance(50)) return Material.RED_MUSHROOM;
        return Material.BROWN_MUSHROOM;
    }

    public Material AirType() {
        if (Chance(AirWebPct())) return Material.WEB;
        return Material.AIR;
    }

    public Material DoorMaterial() {
        if (Chance(IronDoorPct())) return Material.IRON_DOOR_BLOCK;
